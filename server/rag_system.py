...
class RAGSystem:
    def __init__(self):
        ...
        if reasoner_path.exists():
            ...
            print("⚠️ Reasoner model not found. Run train_reasoner.py! Falling back to raw mode.")
    ...
    def get_context(self, query: str):
        ...
        _, indices = self.index.search(vector, 3)
        t_search_end = time.perf_counter()
        
        # 4. Context Assembly
        contexts = [self.chunks[i]["text"] for i in indices[0]]
        final_context = "\n---\n".join(contexts)
